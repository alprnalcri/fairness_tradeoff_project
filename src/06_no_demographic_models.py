...
models = {
    "LogisticRegression_NoDemo": LogisticRegression(
        max_iter=5000,
        random_state=42
    ),

    "DecisionTree_NoDemo": DecisionTreeClassifier(
        random_state=42
    ),

    "RandomForest_NoDemo": RandomForestClassifier(
        n_estimators=300,
        random_state=42,
        n_jobs=-1
    ),

    "XGBoost_NoDemo": XGBClassifier(
        n_estimators=300,
        learning_rate=0.05,
        max_depth=6,
        subsample=0.8,
...
save_path = f"{OUTPUT_DIR}/no_demographic_results.csv"
